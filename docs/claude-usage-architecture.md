...
- **수집되는 것 / 안 되는 것**: 토큰 수·비용·세션·활동 시간·코드 라인 수·편집 수락/거절·커밋/PR 수·요청 메타(모델, 지연, request_id) / **프롬프트·응답·코드 내용·파일 경로·도구 결과는 수집하지 않는다**(해당 이벤트는 저장하지 않고, 프롬프트 내용은 애초에 전송되지 않음). 구성원 안내문에 이 문장을 그대로 쓴다.
