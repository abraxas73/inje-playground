...
정규화(`normalizeName`/`normalizeAgency`): NFKC → 소문자 → 괄호와 그 안 내용 제거 → 공백·기호(`·()[]「」｢｣"'-_.,/`) 제거. 발주기관은 추가로 `(이하 …)`와 영문 약칭 제거. 사업명은 `재공고|긴급|수정|변경|정정` 같은 접미 단어를 떼어낸 `nameCore`도 함께 만든다(유사 판단용).
