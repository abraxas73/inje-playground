...
61. **PDF 제안요청서**(2026-09-09): 나라장터 제안요청서가 PDF뿐인 경우가 있어 `.pdf`도 올릴 수 있다(50MB, 다른 형식과 같은 흐름). 파서 `lib/rfp/parse-pdf.ts`(unpdf = 서버리스용 pdf.js 번들, 워커·canvas 없이 동작).
    - **괘선(테두리 선)으로 표를 복원한다.** 세로선이 열 경계, 가로선이 행 경계이고 경계에 선이 없으면 셀이 이어진 것이라 **셀 병합(rowSpan·colSpan)까지 복원된다**. 선으로 둘러싸인 칸 안의 글자는 그 셀의 텍스트(여러 줄이면 줄바꿈으로 이어 세부 내용 글머리를 살린다), 어떤 칸에도 들지 않는 글자는 문단이다. 좌우 테두리를 그리지 않은 표는 가로선의 양 끝(두 줄 이상이 같은 끝에 닿을 때만)을 첫·마지막 열 경계로 쓴다.
    - **글자 간격으로 표를 추측하지 않는다.** 처음엔 "가로로 크게 벌어지면 다른 칸"으로 만들었는데, 한글 제안요청서 본문은 `사 업 명 : …`처럼 자간을 벌리고 탭으로 정렬해서 **본문 문단이 표로 오인됐다** — 98쪽 문서에서 표 349개가 잡히고 개요 사업명이 "수급인", 발주기관이 "사업명"으로 채워졌다(계약서 양식 표의 머리글). 괘선 기준으로 바꾼 뒤 같은 문서에서 표 158개·요구사항 29건·사업명 정상.
    ...
    - 복원한 뒤는 hwp·docx와 **같은 추출 경로**다: 표준 7행 표면 `standard`, 요건표 꼴이면 `xlsx`, 아니면 Claude 폴백.
    - 한계: 테두리가 없는 표는 문단이 된다(틀린 표를 만드는 것보다 낫다). 표 전체를 한 경로로 그리는 PDF는 표를 놓친다. 한 표가 페이지를 넘으면 페이지마다 다른 표가 된다(아래 이어붙이기로 보완). 그림이 많은 페이지는 도형 테두리가 작은 표로 잡힐 수 있다. **글자가 없는 스캔 이미지 PDF는 415로 거절**한다.
    - SQL `docs/sql/2026-09-09-rfp-pdf-source.sql`(`rfp_files.format`에 pdf 추가, 운영 적용 완료).
62. **요구사항 표·개요 인식 확장**(2026-09-09, PDF 실측에서 나온 것이지만 hwp·docx에도 적용된다):
    1. **분류 행이 없는 변형 표**: 첫 셀이 "요구사항 번호"로 시작하고 명칭·상세(정의/세부 내용)만 있는 표도 요구사항 표로 읽는다(`isRequirementTable`). 한전 AI 인프라 제안요청서가 이 꼴이다 — 전에는 표준으로 인식되지 않아 엑셀 요건표 규칙으로 잘못 흘러가 요구사항 113건이 쓰레기로 만들어졌다. 구분 코드는 ID 접두어에서, **구분명은 총괄표에서** 찾는다(없으면 코드).
    2. **부여규칙 열이 없는 총괄표**: "구 분 | 세부 내용 | 수량"처럼 ID 부여규칙 열이 없으면 구분명 괄호 안의 약어를 코드로 쓴다 — "성능 요구사항(PER, Performance Requirement)" → PER. 코드가 없는 행은 버리므로 "구분 | 품목 | 수량" 물량표는 총괄표로 오인되지 않는다. 덕분에 구분 탭 이름과 **건수 대조 경고**가 이 양식에서도 동작한다(실측: "총괄표 ECR 15건, 추출 12건" — 문서 자체의 목록은 13건이라 원문이 어긋난 경우도 드러난다).
    3. **페이지로 갈린 이어지는 표**: 명칭 행이 없고 번호만 있는 조각 표는 같은 ID 요구사항의 세부 내용에 이어 붙이고 경고를 남긴다(`appendContinuation`). PDF는 표가 페이지를 넘으면 페이지마다 다른 표가 되기 때문에 그대로 두면 그 내용이 사라진다.
    4. **개요 라벨**: 자간을 벌린 라벨(`사 업 명`)을 받도록 공백을 모두 지운 뒤 비교하고, 표 라벨 칸의 끝 콜론(`○ 공고명:`)을 떼고, **공고명**을 사업명 라벨에 더했다. 값 칸이 또 다른 라벨이면(빈 계약서 양식의 머리글) 값으로 쓰지 않는다. 실측: 조달청 공고서에서 사업명이 「국가를 당사자로 하는 계약에 관한 법률」 → "2024년 건강보험 클라우드 표준 플랫폼 구축 사업"으로 바뀌고 발주기관·금액·계약방법도 정상 인식.
    5. **요건표 헤더 오인 방지**: 헤더 행으로 인정하려면 인식한 열이 **2개 이상**이어야 한다(`findHeader`). "…기본 요건"처럼 항목 열 문구가 값 칸에 우연히 들어간 라벨/값 표가 요건표로 잡히던 것을 막는다.

63. **확정 작업(리뷰 큐)**(2026-09-12, `/rfp/[id]/review`): 상세 화면 판정 요약 옆 "확정 작업 · 미결 N" 버튼으로 들어간다. 매핑 단위(요구사항 × 세부 항목 — `groupRowsByDetail`과 같은 단위)를 왼쪽 목록에서 하나씩 넘기며 오른쪽에서 결정한다. 후보는 점수순으로 나열되고 **하나를 충족/부분충족으로 확정하면 같은 단위의 다른 후보는 지워진다**(후보는 규칙 엔진이 다시 만들 수 있어 잃는 게 없다). 후보가 맞지 않으면 단위를 설계·구축영역/해당없음으로 닫는다(후보 전부 삭제 + 수동 행 하나). 키보드: `j`/`k` 후보 이동, `1`/`2` 충족/부분충족, `b`/`n` 설계·구축영역/해당없음, `←`/`→`(스페이스) 단위 이동. 결정하면 다음 미결 단위로 자동 이동. 진행률 막대 = 확정 단위 / 전체 단위(행이 없는 요구사항도 "미매핑" 단위로 분모에 들어간다 — 결정해야 할 것 전체가 분모여야 한다). "미결만/전체" 필터. 설명·근거 URL·메모 편집은 여기서 하지 않고 상세 화면 편집기에서 한다(이 화면은 결정만).
    - 왜 별도 API인가: 판정 조합 규칙이 "설계·구축영역/해당없음은 충족·부분충족·후보와 함께 둘 수 없다"라서 단위를 닫으려면 후보를 먼저 지워야 한다. 그 순서를 화면이 PATCH/DELETE로 맡으면 중간에 끊겼을 때 반쪽 상태가 남으므로 `POST …/mapping/decide`가 한 번에 처리한다(규칙 검사는 편집기와 같은 `validateManualMapping`). 이미 확정 행이 있는 단위는 닫을 수 없다(400 — 편집기에서 그 행을 먼저 지우거나 바꾸라고 안내).
    - 단위 상태(`lib/rfp/mapping/review.ts` `unitStatus`): 확정 행(후보 아님)이 있으면 그중 가장 좋은 판정, 후보만 있으면 **검토 대기**, 행이 없으면 **미매핑**. 화면·API·xlsx 대응표/Gap 시트가 같은 정의를 쓴다.
    - 배경: 리뷰(2026-09-11) 시점 프로덕션 매핑 4,182행 중 확정 1행 — 표에서 행을 펼쳐 select를 바꾸는 편집기로는 수백 건을 확정할 사람이 없었다. 확정 데이터가 쌓여야 재사용 제안(64항)과 카탈로그 잡음 정리가 돌아간다.
64. **확정 매핑 재사용**(2026-09-12): 확정 작업 화면의 "이전 확정 제안" 카드. 다른 요구사항(다른 프로젝트 포함, 자기 요구사항 제외)에서 사람이 확정한 행(`edited=true`·후보 아님) 중 **요구사항 명칭 + 세부 항목 문장이 비슷한 것**을 최대 3개 보여주고, "이 확정 적용"을 누르면 그 행의 솔루션·기능·판정·근거 문장을 이 단위에 복사한다(수동 행, 설명 앞에 "이전 확정 재사용(프로젝트 요구사항ID)"을 남겨 출처를 알 수 있게). 비교는 규칙 엔진과 같은 문자 bigram cosine(`lib/rfp/mapping/reuse.ts`, 임계 0.45 — 문장이 거의 같아야 뜬다, 틀린 제안이 많으면 사람이 제안을 믿지 않게 된다), 같은 (솔루션, 기능, 판정)은 가장 비슷한 것 하나만. 공공 RFP는 보안·성능·프로젝트 관리 요구사항 문장이 문서를 넘어 반복되므로 확정 1건이 다음 문서의 여러 건을 줄인다 — 규칙 엔진 키워드 품질 문제를 우회하는 길이기도 하다. API `GET …/reuse?requirementId&detailKey`, 적용은 `decide {action:"reuse", sourceMappingId}`.
