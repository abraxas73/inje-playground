...
41. 구분 탭(열 10개)에서 표가 옆으로 스크롤되지 않는다(열 폭이 rem 비율 → %). 펼친 행의 매핑 패널도 화면 안에서 줄바꿈. 규칙 엔진 행의 "자동 매칭 — 일치 키워드 … 유사도" 설명은 별도 칸 없이 판정·솔루션·기능 선택 줄 끝에 회색 한 줄로 붙고, 클릭하면 편집 칸이 열린다(Claude·수동 행은 설명 칸 그대로).
