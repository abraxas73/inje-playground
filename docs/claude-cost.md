...
- **표시 통화 토글**(화면 우상단 `$ 달러` / `₩ 원`, 브라우저에 기억): 저장·집계는 언제나 USD 센트이고, 원화는 시스템 설정의 환율(`usd_krw_rate`)을 곱해 표시할 때만 만든다(원 단위 반올림). 카드·표·조직별 상세·인보이스 표·API 비용·CSV(`_usd`/`_krw` 접미)까지 같이 바뀐다. 같은 토글이 **Claude Code 사용량·Claude 사용량(Chat/Cowork)·개인용 `/usage/code`·`/usage/chat`** 화면 우상단에도 있어 추정 비용(OTel `cost_usd`)·초과 지출(CSV)을 원으로 볼 수 있다(공용 `components/shared/currency-context.tsx`의 `useMoney()`; 프로바이더가 없는 화면(성과)은 달러 고정). 은행 환율로 청구된 실제 원화 카드 금액과는 다를 수 있다.
