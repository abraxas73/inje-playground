...
RSS 요약은 기사 첫 문단만 담아서 인사 기사 상당수가 `◇ 과장급 전보` 같은 **분류 머리글만** 남는다(2026-09-17 기준 인사 50건 중 32건). 그래서 요약에 항목 줄(`▲ …`)이 없고 60자 미만일 때만 원문을 읽어 **항목 한두 줄**을 덧붙인다(`article.ts`). 새 기사이거나 저장된 요약이 아직 머리글뿐인 기사만 대상이라 한 번 보강되면 다시 읽지 않고, 1회 수집당 12건·동시 4건·건당 8초 상한이며 실패하면 RSS 요약을 그대로 둔다. 전문은 저장하지 않는다. 이미 보강된 저장분은 이후 수집이 RSS 요약으로 되돌리지 않는다. 대상은 **그때 RSS에 있는 기사**뿐이라, 기능 적용 전에 쌓인 과거분(2026-09-17 21건)은 같은 규칙으로 1회 backfill했다. RSS 제공 기간 이전의 과거 기사는 자동 복원되지 않는다.
