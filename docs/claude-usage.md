...
- **등록(조직별)**: claude.ai 관리자 설정 > 제품 > Office Agents > 모니터링 — OTLP 엔드포인트 `https://inje-playground.vercel.app/api/otel`(추가 기능이 `/v1/traces`를 붙임), 프로토콜 `http/json`, 헤더 `Authorization=Bearer <CLAUDE_OFFICE_OTEL_TOKEN>`. 설정은 조직 단위라 좌측 상단 조직 드롭다운으로 바꿔가며 조직마다 넣어야 한다. 추가 기능을 다시 열 때부터 적용. 2026-09-07 7개 조직 모두 등록 완료(API로 대조).
...
- **집계**: RPC `claude_office_usage(p_from, p_to, p_emails, p_org)`(`docs/sql/2026-09-07-claude-office-daily.sql`) — `agent.query`(턴, 사용자·표면·세션) 기준으로 자식 `agent.stream`(모델 호출·토큰)·`agent.tool_execution`(도구)을 trace_id로 귀속, `file.upload`는 session.id로 귀속. 루트 스팬이 아직 오지 않은 트레이스는 빠진다. 화면: 어드민 `/admin/claude-chat`(Claude 사용량 Chat/Cowork) "Office Agents" 탭(`OfficeUsageTab` → `GET /api/admin/claude-usage/office`)과 멤버 활동·팀별 집계 표의 "Office 턴" 컬럼(members API가 같은 데이터 기간의 턴을 붙임), 개인 `/usage/chat` Office 턴 컬럼·Office Agents 카드(`GET /api/usage/office`, 스코프 이메일만). Claude Code 화면이 아니라 Claude 사용량(제품별) 화면에 둔다 — Office Agents는 별도 제품이기 때문. 공용 패널 `OfficeUsagePanel`, 집계 함수 `lib/claude-usage/office-usage.ts`.
