...
## 페이지 접근 권한 연동 (2026-09-16)

`/admin/page-permissions`에 마케팅 Master DB 행을 추가했다. 기본은 차단이며 관리자가 사용자(관리자 포함)에게 개별 허용할 수 있다. 허용된 사용자는 Master 조회와 Contact 제출이 가능하고, 승인·반려는 여전히 고정 관리자 또는 검수자 지정을 받은 계정만 할 수 있다. 고정 관리자·검수자는 권한 설정과 무관하게 접근한다. 상세는 `docs/page-access.md`.
