...
| pdf | `parse-pdf.ts` (unpdf = 서버리스용 pdf.js, **괘선으로 표 복원**) | 복원한 표로 위 세 경로 그대로 |
...
PDF에는 표 구조가 없어 **괘선(테두리 선)으로 표를 복원한다**(세로선=열 경계, 가로선=행 경계, 경계에 선이 없으면 셀 병합). 글자 간격으로 표를 추측하면 자간을 벌린 한글 본문이 표로 오인되므로 쓰지 않는다. 테두리가 없는 표는 문단이 되고, 글자가 없는 스캔 PDF는 415로 거절한다. 자세한 규칙·한계는 런북 61·62항.
...
실행 스코프는 세 가지다. 프로젝트 전체(전체 다시 매핑·미매핑만), 요구사항 하나, 세부 항목 하나. 같은 실행 레이어를 쓰고 `requirementIds`·`detailKey`만 달라진다. 스코프 실행은 그 범위의 자동 행만 지우고 다시 넣으므로 사람이 고친 행과 다른 항목은 건드리지 않는다. 후보 상한은 어드민 설정이 기본값이지만 레이어에서 1~5로 바꿔 그 실행에만 적용할 수 있다.
...
- PDF는 괘선으로 표를 복원하므로 테두리가 없는 표를 놓치고, 표가 페이지를 넘으면 페이지마다 다른 표가 된다(번호만 있는 조각 표는 앞 요구사항 세부 내용에 이어 붙인다 — 런북 61·62항).
