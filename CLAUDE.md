...
- `/rfp`, `/rfp/[id]` — RFP 분석(user): 제안요청서(hwp·hwpx·docx) 또는 엑셀 요건표(xlsx — 견적요청서·기술검토표처럼 한 행 = 한 요구사항) 업로드 → 프로젝트 등록(중복 판단) → 요구사항 표(TanStack Table 셀 편집·행 추가/삭제; 구분 탭에 총괄표 분류명 표시·검색 — `lib/rfp/category-summary.ts`; 매핑 후 요구사항 ID는 판정 색 버튼 → 클릭 시 행 펼침, ID 편집은 펼친 패널 헤더) → xlsx 다운로드 → 솔루션 매핑(상세 "솔루션 매핑 실행" 다이얼로그에서 대상 솔루션 체크박스(기본 모두)·엔진 규칙(키워드, 기본)/Claude 선택, 요구사항별 판정 충족/부분충족/후보/설계·구축영역/해당없음, 행 펼침 편집 — 근거는 URL 대신 소스 문서 제목·기능 요약 카드 + 바로가기) → SharePoint 등록(3단계: 상세 "SharePoint 등록" 섹션에서 폴더 '링크 복사' 값 지정 → 같은 xlsx를 사용자 위임 권한으로 업로드(같은 날 덮어쓰기) → 이력·Teams 채널 알림). 런북 `docs/rfp-analyzer.md`
