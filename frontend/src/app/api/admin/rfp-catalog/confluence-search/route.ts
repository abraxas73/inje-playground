...
import { searchConfluencePages, SEARCH_LIMIT_DEFAULT, type SearchScope } from "@/lib/rfp/catalog/confluence-search";

export const runtime = "nodejs";

/** GET /api/admin/rfp-catalog/confluence-search?q=&limit=&scope=title|text → {results: ConfluenceSearchHit[]} (4단계 §4.6). admin. */
export async function GET(request: NextRequest) {
  ...
  const scope: SearchScope = request.nextUrl.searchParams.get("scope") === "text" ? "text" : "title";
  try {
    return NextResponse.json({ results: await searchConfluencePages(cfg, q, limit, fetch, scope) });
  } catch (e) {
