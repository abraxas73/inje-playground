...
export async function POST(request: NextRequest, { params }: Params) {
  ...
  if (engine === "llm" && !process.env.ANTHROPIC_API_KEY) return NextResponse.json({ error: "ANTHROPIC_API_KEY가 설정되지 않았습니다." }, { status: 400 });
  ...
  // 대상 솔루션(화면 체크박스). 비었으면 활성 전체.
  const wanted = Array.isArray(body.solutions) ? body.solutions.filter((s): s is string => typeof s === "string" && s.trim() !== "").map((s) => s.trim()) : [];
  let catalogEmpty: boolean;
  let solutionCodes: string[] = [];
  try {
    const all = await loadCatalog(auth.admin, { activeSolutionsOnly: true });
    const scoped = scopeCatalog(all, wanted);
    if (wanted.length && !scoped.length) {
      return NextResponse.json({ error: "선택한 솔루션을 찾을 수 없습니다. 대상 솔루션을 다시 고르세요." }, { status: 400 });
    }
    solutionCodes = wanted.length ? scoped.map((s) => s.code) : [];
    catalogEmpty = createRulesEngine(scoped).lookup.size === 0;
  } catch (e) {
