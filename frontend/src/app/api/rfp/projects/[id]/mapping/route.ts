...
/**
 * POST /api/rfp/projects/[id]/mapping {mode?: "all"|"missing", engine?: "rules"|"llm"(기본 rules), confirm?: boolean}  (2단계 §4.3 + 4단계 §5.5)
...
export async function POST(request: NextRequest, { params }: Params) {
  ...
  if (!auth.ok) return auth.response;
  const { id } = await params;
  const body = (await request.json().catch(() => ({}))) as { mode?: unknown; engine?: unknown; confirm?: unknown };
  ...
  try {
    catalogEmpty = createRulesEngine(await loadCatalog(auth.admin, { activeSolutionsOnly: true })).lookup.size === 0;
  } catch (e) {
