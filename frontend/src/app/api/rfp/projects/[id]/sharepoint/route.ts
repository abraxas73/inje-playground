...
export async function GET(_request: NextRequest, { params }: Params) {
  ...
  try {
    const [uploads, defaultFolder] = await Promise.all([loadUploads(auth.admin, id, 20), loadUserDefaultFolder(auth.admin, auth.userId)]);
    const res: SharepointResponse = { folder: parseSharepointFolder(project.sharepoint_folder), defaultFolder, lastUpload: uploads[0] ?? null, uploads };
