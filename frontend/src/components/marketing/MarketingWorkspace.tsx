...
interface Meta { total: number; pending: number; missingCompany: number; missingName: number; unclassified: number; reviewer: boolean; admin: boolean; identity: { name: string; division: string } }
...
export default function MarketingWorkspace() {
  ...
  const [form, setForm] = useState<{ initial?: ContactData; initialClear?: Field[]; dbId?: string } | null>(null); const [upload, setUpload] = useState<"master" | "submit" | null>(null);
  const [contact, setContact] = useState<{ contact: Contact; sources: unknown[]; events: ReviewEvent[] } | null>(null); const [organization, setOrganization] = useState<Organization | null>(null); const [adminOpen, setAdminOpen] = useState(false);
  ...
  async function resubmit(s: Submission) {
    try {
      ...
      setForm({ initial: s.data, initialClear: s.clear_fields, dbId: target?.contact.db_id });
  ...
  return <div className="mx-auto w-full min-w-0 max-w-[1440px] space-y-6">
    ...
    {busy ? <div role="status" className="rounded-xl border bg-card p-12 text-center text-muted-foreground">데이터를 불러오고 있습니다…</div> : !listing.rows.length ? <div className="rounded-xl border bg-card py-16 text-center space-y-3"><Database className="mx-auto h-9 w-9 text-muted-foreground/50" /><h2 className="font-medium">{error ? "데이터를 불러오지 못했습니다" : view === "queue" ? "표시할 제출이 없습니다" : "표시할 데이터가 없습니다"}</h2><p className="text-sm text-muted-foreground">{view === "queue" ? "Contact 등록 또는 Excel 제출로 검수 요청을 시작하세요." : "검색 조건을 확인하거나 Contact를 등록해 주세요."}</p>{meta?.admin && view === "master" && meta.total === 0 && <Button variant="outline" onClick={() => setUpload("master")}>기존 Master Excel 이관</Button>}</div> : <>
      ...
      {view === "organizations" && <div className="overflow-auto rounded-xl border bg-card"><table className="w-full text-sm text-left"><thead className="bg-muted/50"><tr>{["표준 회사·기관명", "분류", "확인 상태", "승인 별칭", "Contact", ""].map((t,i) => <th key={i} className="p-4 font-medium">{t}</th>)}</tr></thead><tbody>{(listing.rows as Organization[]).map(o => <tr key={o.id} className="border-t"><td className="p-4 font-medium">{o.name}</td><td className="p-4"><KindBadge kind={o.category} /></td><td className="p-4 whitespace-nowrap">{o.review_status === "confirmed" ? "확인 완료" : "확인 필요"}</td><td className="p-4 text-muted-foreground">{o.aliases.join(", ") || "—"}</td><td className="p-4">{listing.counts?.[o.id] ?? 0}명</td><td className="p-4">{meta?.reviewer && <Button variant="outline" size="sm" onClick={() => setOrganization(o)}>기준 수정</Button>}</td></tr>)}</tbody></table></div>}
    ...
    {contact && <Dialog open onOpenChange={() => setContact(null)}><DialogContent className="sm:max-w-2xl max-h-[90vh] overflow-auto"><DialogHeader><DialogTitle>{contact.contact.db_id} · {contact.contact.data.name || "성명 미입력"}</DialogTitle><DialogDescription>Master 상세 정보 및 원본 출처</DialogDescription></DialogHeader><dl className="grid grid-cols-[120px_1fr] gap-3 text-sm">{Object.entries(FIELDS).map(([k,t]) => <div key={k} className="contents"><dt className="text-muted-foreground">{t}</dt><dd className="break-all">{visibleData(contact.contact)[k as keyof ContactData] || "—"}</dd></div>)}</dl><details><summary className="text-sm cursor-pointer">최초 이관 원본</summary><pre className="text-xs max-h-64 overflow-auto whitespace-pre-wrap break-all">{JSON.stringify(contact.sources, null, 2)}</pre></details><h3 className="text-sm font-semibold">Contact 및 소속 회사 변경 이력</h3><HistoryList events={contact.events ?? []} /><Button onClick={() => { setForm({ initial: visibleData(contact.contact), dbId: contact.contact.db_id }); setContact(null); }}>변경 정보 제출</Button></DialogContent></Dialog>}
    ...
    {adminOpen && <AdminSettings onClose={() => setAdminOpen(false)} onImport={() => { setAdminOpen(false); setUpload("master"); }} empty={meta?.total === 0} />}
...
function OrganizationEditor({ org, count, onClose, onSaved }: { org: Organization; count: number; onClose: () => void; onSaved: () => void }) {
  const [reviewStatus,setReviewStatus] = useState(org.review_status ?? "pending"); const [name,setName] = useState(org.name); const [category,setCategory] = useState(org.category); const [aliases,setAliases] = useState(org.aliases.join("\n")); const [reason,setReason] = useState(""); const [busy,setBusy] = useState(false); const [error,setError] = useState("");
  async function save() { setBusy(true); try { await api("/api/marketing/organizations", { id: org.id, version: org.version, name, category, aliases: aliases.split("\n").map(a => a.trim()).filter(Boolean), reason, reviewStatus }); onSaved(); } catch(e) { setError(errorMessage(e)); } finally { setBusy(false); } }
  return <Dialog open onOpenChange={v => { if (!v && !busy) onClose(); }}><DialogContent><DialogHeader><DialogTitle>회사·기관 기준 수정</DialogTitle><DialogDescription>소속 Contact {count}명에 동일한 표준명과 분류가 적용됩니다.</DialogDescription></DialogHeader><label className="text-sm space-y-1">표준명<Input value={name} onChange={e => setName(e.target.value)} /></label><label className="text-sm space-y-1">분류<select className={`${selectClass} w-full`} value={category} onChange={e => { setCategory(e.target.value); setReviewStatus("pending"); }}>{CATEGORIES.map(c => <option key={c}>{c}</option>)}</select></label><label className="text-sm space-y-1">승인 별칭 (한 줄에 하나)<textarea className="w-full border rounded-md p-2" value={aliases} onChange={e => setAliases(e.target.value)} /></label><label className="text-sm space-y-1">확인 상태<select className={`${selectClass} w-full`} value={reviewStatus} onChange={e => setReviewStatus(e.target.value as "pending" | "confirmed")}><option value="pending">확인 필요</option><option value="confirmed" disabled={category === "확인 필요"}>회사·분류 확인 완료</option></select></label><label className="text-sm space-y-1">변경 사유 / 별도 법인 판단 근거<Input value={reason} onChange={e => setReason(e.target.value)} /></label>{error && <p role="alert" className="text-destructive text-sm">{error}</p>}<Button disabled={busy || !name.trim() || reason.trim().length < 2} onClick={save}>{busy ? "저장 중…" : "확인 후 기준 변경"}</Button></DialogContent></Dialog>;
}
function AdminSettings({ onClose, onImport, empty }: { onClose: () => void; onImport: () => void; empty: boolean }) {
  const [email,setEmail] = useState(""); const [message,setMessage] = useState(""); const [busy,setBusy] = useState(false);
  async function setReviewer(enabled: boolean) { setBusy(true); try { await api("/api/marketing/reviewers", { email, enabled }); setMessage(enabled ? "검수자로 지정했습니다." : "검수자 권한을 해제했습니다."); } catch(e) { setMessage(errorMessage(e)); } finally { setBusy(false); } }
  return <Dialog open onOpenChange={v => { if (!v && !busy) onClose(); }}><DialogContent><DialogHeader><DialogTitle>마케팅 DB 관리</DialogTitle><DialogDescription>관리자는 기본 검수 권한을 가지며, 담당자 계정을 추가로 지정할 수 있습니다.</DialogDescription></DialogHeader><label className="text-sm space-y-1">검수 담당자 계정 이메일<Input type="email" value={email} onChange={e => setEmail(e.target.value)} /></label><div className="flex gap-2"><Button disabled={busy || !email.trim()} onClick={() => setReviewer(true)}>검수자 지정</Button><Button variant="outline" disabled={busy || !email.trim()} onClick={() => setReviewer(false)}>지정 해제</Button></div>{message && <p role="status" className="text-sm">{message}</p>}<div className="border-t pt-4"><Button variant="outline" disabled={!empty} onClick={onImport}>기존 Master Excel 최초 이관</Button><p className="mt-2 text-xs text-muted-foreground">최초 이관은 Master가 비어 있을 때만 가능합니다. 기존 데이터의 누락값과 원본 18개 필드를 보존합니다.</p></div></DialogContent></Dialog>;
}
