...
export default function MonthOrgDetail({ month }: { month: MonthlyCost }) {
  const columns: Column<MonthlyOrgCost>[] = [
        ...
        <div className="space-y-0.5 text-xs">
          {o.invoices.map((i) => {
            ...
            <div key={i.id} className="flex flex-wrap items-center gap-x-2">
              ...
              {a && a.days !== null && a.totalDays !== null && <span className="text-muted-foreground">{formatCents(i.total_cents)} 중 <span className="text-foreground">{formatCents(a.cents)}</span> ({a.days}/{a.totalDays}일)</span>}
    ...
    { key: "total", header: month.basis === "period" ? "배분액" : "청구 총액", value: (o) => o.totalCents / 100, align: "right", render: (o) => formatCents(o.totalCents), total: (rows) => formatCents(rows.reduce((a, r) => a + r.totalCents, 0)) },
