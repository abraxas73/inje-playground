...
export default function MonthOrgDetail({ month }: { month: MonthlyCost }) {
  ...
  const columns: Column<MonthlyOrgCost>[] = [
    ...
    { key: "invoices", header: "인보이스", value: (o) => o.invoices.length,
      render: (o) => (o.invoices.length === 0 ? <span className="text-destructive">없음</span> : (
        <div className="space-y-0.5 text-xs">
          {o.invoices.map((i) => {
            const a = o.allocations.find((x) => x.invoiceId === i.id);
            return (
            <div key={i.id} className="flex flex-wrap items-center gap-x-2">
              ...
              {a && a.days !== null && a.totalDays !== null && <span className="text-muted-foreground">{fmt(i.total_cents)} 중 <span className="text-foreground">{fmt(a.cents)}</span> ({a.days}/{a.totalDays}일)</span>}
    ...
    { key: "total", header: month.basis === "period" ? "배분액" : "청구 총액", value: (o) => o.totalCents / 100, align: "right", render: (o) => fmt(o.totalCents), total: (rows) => fmt(rows.reduce((a, r) => a + r.totalCents, 0)) },
    { key: "perSeat", header: "좌석당", value: (o) => (o.seats ? o.totalCents / 100 / o.seats : null), align: "right", render: (o) => (o.seats ? fmt(o.totalCents / o.seats) : "—") },
    { key: "active", header: "Claude Code\n활성 사용자", hint: ACTIVE_USERS_HINT, value: (o) => o.activeUsers, align: "right", total: "sum" },
    { key: "perUser", header: "1인당", value: (o) => (o.activeUsers ? o.totalCents / 100 / o.activeUsers : null), align: "right", render: (o) => (o.activeUsers ? fmt(o.totalCents / o.activeUsers) : "—") },
    { key: "est", header: "추정 비용\n(OTel)", hint: EST_COST_HINT, value: (o) => o.estCostUsd, align: "right", render: (o) => fmtUsd(o.estCostUsd), total: (rows) => fmtUsd(rows.reduce((a, r) => a + r.estCostUsd, 0)) },
  ];
  return (
    <div className="space-y-2">
      <div className="flex flex-wrap items-center gap-2 text-sm">
        <span className="font-medium">{month.month} 조직별{month.basis === "period" ? " · 서비스 기간 일할" : " · 발행일 기준"}</span>
        {month.missingOrgs.length > 0 && <Badge variant="outline" className="text-destructive border-destructive/40" title={MISSING_HINT}>기간 미커버: {month.missingOrgs.join(", ")}</Badge>}
        {month.unassignedCents > 0 && <Badge variant="destructive">미배정 {fmt(month.unassignedCents)}</Badge>}
      </div>
