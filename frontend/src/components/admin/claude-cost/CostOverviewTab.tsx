...
/** 티어별 좌석 한 줄 요약: "Premium 202 · Standard 114" */
const tierSeatsText = (m: MonthlyCost): string => tiersWithSeats(m).map((t) => `${tierLabel(t.plan)} ${int(t.seats ?? 0)}`).join(" · ");

/** 월별 청구(인보이스) vs 사용량(OTel·CSV). 기간·조직·월 기준·표시 통화를 고르고, 월 행을 누르면 조직별 상세 */
export default function CostOverviewTab({ onMeta }: { onMeta: (m: { apiCostAvailable: boolean; orgs: ClaudeOrg[] }) => void }) {
  const { fmt, fmtUsd, csv, suffix } = useCurrency();
  const cell = (v: number | null): string => (v === null ? "—" : fmt(v));
  const tierPerSeatText = (m: MonthlyCost): string => tiersWithSeats(m).map((t) => `${tierLabel(t.plan)} ${cell(tierPerSeat(t))}`).join(" · ");
  ...
  const columns: Column<MonthlyCost>[] = [
    ...
    { key: "subtotal", header: "청구 세전", value: (m) => m.billed.subtotalCents / 100, align: "right", render: (m) => fmt(m.billed.subtotalCents), total: (rs) => fmt(rs.reduce((a, r) => a + r.billed.subtotalCents, 0)) },
    { key: "tax", header: "VAT", value: (m) => m.billed.taxCents / 100, align: "right", render: (m) => fmt(m.billed.taxCents), total: (rs) => fmt(rs.reduce((a, r) => a + r.billed.taxCents, 0)) },
    { key: "total", header: "청구 총액", hint: BILLED_HINT, value: (m) => m.billed.totalCents / 100, align: "right", className: "font-semibold", render: (m) => fmt(m.billed.totalCents), total: (rs) => fmt(rs.reduce((a, r) => a + r.billed.totalCents, 0)) },
    ...(showApi ? [{ key: "api", header: "API 비용\n(Console)", hint: API_COST_HINT, value: (m: MonthlyCost) => (m.apiCostCents ?? 0) / 100, align: "right" as const, render: (m: MonthlyCost) => fmt(m.apiCostCents ?? 0), total: (rs: MonthlyCost[]) => fmt(rs.reduce((a, r) => a + (r.apiCostCents ?? 0), 0)) }] : []),
    { key: "est", header: "추정 비용\n(OTel)", hint: EST_COST_HINT, value: (m) => m.usage.estCostUsd, align: "right", render: (m) => fmtUsd(m.usage.estCostUsd), total: (rs) => fmtUsd(rs.reduce((a, r) => a + r.usage.estCostUsd, 0)) },
    ...
    { key: "perSeat", header: "좌석당", hint: PER_SEAT_HINT, value: (m) => (perSeat(m) === null ? null : (perSeat(m) as number) / 100), align: "right", render: (m) => (perSeat(m) === null ? "—" : (
      <span className="inline-flex flex-col items-end leading-tight">{tiersWithSeats(m).length > 1 ? <><span className="whitespace-nowrap">{tierPerSeatText(m)}</span><span className="text-[10px] text-muted-foreground">평균 {cell(perSeat(m))}</span></> : cell(perSeat(m))}</span>)) },
    { key: "perUser", header: "1인당", hint: PER_USER_HINT, value: (m) => (perUser(m) === null ? null : (perUser(m) as number) / 100), align: "right", render: (m) => cell(perUser(m)) },
  ];

  const exportCsv = () => {
    const tierCol = (m: MonthlyCost, f: (t: MonthlyTier) => string | number) => m.tiers.map((t) => `${tierLabel(t.plan)}:${f(t)}`).join(";");
    const money = (cents: number | null) => (cents === null ? "" : csv(cents));
    const head = ["month", "invoices", "seats", "seats_by_tier", `subtotal_${suffix}`, `vat_${suffix}`, `total_${suffix}`, `total_by_tier_${suffix}`, `per_seat_by_tier_${suffix}`, ...(showApi ? [`api_cost_${suffix}`] : []), `est_cost_${suffix}`, "code_active_users", "csv_active_members", "sessions", "prompts_human", `per_seat_${suffix}`, `per_user_${suffix}`, "missing_orgs"];
    downloadCsv(`claude-cost-monthly-${months}m-${basis}-${suffix}.csv`, head, rows.map((m) => [m.month, m.invoices, m.seats ?? "", tierCol(m, (t) => t.seats ?? ""), money(m.billed.subtotalCents), money(m.billed.taxCents), money(m.billed.totalCents), tierCol(m, (t) => money(t.totalCents)), tierCol(m, (t) => money(tierPerSeat(t))), ...(showApi ? [money(m.apiCostCents ?? 0)] : []), money(Math.round(m.usage.estCostUsd * 100)), m.usage.activeUsers, m.csvActiveMembers ?? "", m.usage.sessions, m.usage.promptsHuman, money(perSeat(m)), money(perUser(m)), m.missingOrgs.join("; ")]));
  };
      ...
      {latest && (
        <div className="grid gap-3 sm:grid-cols-2 lg:grid-cols-4">
          ...
          <Card><CardHeader className="pb-1"><CardTitle className="text-xs text-muted-foreground">좌석당 월 비용{tiersWithSeats(latest).length > 1 ? " (티어별)" : ""}</CardTitle></CardHeader><CardContent className="text-2xl font-semibold">{tiersWithSeats(latest).length > 1 ? (
            <div className="space-y-0.5">{tiersWithSeats(latest).map((t) => <div key={t.plan} className="flex items-baseline gap-2"><span className="text-xs font-normal text-muted-foreground w-20">{tierLabel(t.plan)}</span><span>{cell(tierPerSeat(t))}</span><span className="text-xs font-normal text-muted-foreground">{int(t.seats ?? 0)}석 · {fmt(t.totalCents)}</span></div>)}<div className="text-xs font-normal text-muted-foreground">평균 {cell(perSeat(latest))}</div></div>
          ) : cell(perSeat(latest))}</CardContent></Card>
          <Card><CardHeader className="pb-1"><CardTitle className="text-xs text-muted-foreground">활성 사용자당</CardTitle></CardHeader><CardContent className="text-2xl font-semibold">{cell(perUser(latest))}<div className="text-xs text-muted-foreground">Claude Code 활성 {int(latest.usage.activeUsers)}명{latest.csvActiveMembers !== null ? ` · CSV 활성 ${int(latest.csvActiveMembers)}명` : ""}</div></CardContent></Card>
        </div>
      )}
