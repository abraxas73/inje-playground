...
export default function CostOverviewTab({ onMeta }: { onMeta: (m: { apiCostAvailable: boolean; orgs: ClaudeOrg[] }) => void }) {
  ...
  const columns: Column<MonthlyCost>[] = [
    ...
    { key: "perSeat", header: "좌석당", hint: PER_SEAT_HINT, value: (m) => (perSeat(m) === null ? null : (perSeat(m) as number) / 100), align: "right", render: (m) => centsCell(perSeat(m)) },
    ...
    <div className="space-y-4">
      ...
      {latest && (
        <div className="grid gap-3 sm:grid-cols-2 lg:grid-cols-4">
          ...
          <Card><CardHeader className="pb-1"><CardTitle className="text-xs text-muted-foreground">좌석당 월 비용</CardTitle></CardHeader><CardContent className="text-2xl font-semibold">{centsCell(perSeat(latest))}</CardContent></Card>
