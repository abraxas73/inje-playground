...
/**
 * 조직 필터 파라미터 → org_id 목록. `all`(또는 없음)은 null(필터 없음), `personal`은 category='personal'인 조직 전체,
 * 그 외는 그 id 하나. 테이블 조회는 applyOrgFilter로, RPC(claude_code_tool_summary·claude_code_hourly)는 SQL 안에서 'personal'을 해석한다.
 */
export async function resolveOrgIds(admin: SupabaseClient, org: string | null | undefined): Promise<string[] | null> {
  ...
  if (org === "personal") {
    const { data, error } = await admin.from("claude_orgs").select("id").eq("category", "personal");
