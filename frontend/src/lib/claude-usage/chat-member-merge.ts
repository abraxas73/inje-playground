 ...
 * Claude Code 프롬프트(code_prompts)와 Office 턴(office_turns)은 이메일 기준으로 이미 합산된 값이라 더하지 않고 최댓값을 쓴다.
 */
...
export function mergeMembersByEmail<T extends MergeableMemberRow>(rows: T[]): T[] {
  ...
  for (const r of rows) {
    ...
    prev.office_turns = Math.max(prev.office_turns ?? 0, r.office_turns ?? 0);
  }
  return [...map.values()];
}
