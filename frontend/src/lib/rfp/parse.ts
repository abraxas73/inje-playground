 ...
 */
export const UPLOAD_PATH_RE = /^uploads\/[0-9a-f]{8}-[0-9a-f]{4}-[0-9a-f]{4}-[0-9a-f]{4}-[0-9a-f]{12}\/[0-9a-f]{8}-[0-9a-f]{4}-[0-9a-f]{4}-[0-9a-f]{4}-[0-9a-f]{12}\.(?:hwp|hwpx|docx|xlsx|pdf)$/;
...
export const ALLOWED_EXTENSIONS = ["hwp", "hwpx", "docx", "xlsx", "pdf"] as const;
/** 사람이 읽는 허용 형식 목록(업로드 안내·오류 문구 공용) */
export const ALLOWED_EXTENSIONS_TEXT = "hwp·hwpx·docx·xlsx·pdf";
...
export function detectFormat(buf: Buffer, fileName: string): DocumentFormat {
  ...
  // PDF 서두(%PDF-)는 파일 맨 앞이 아니어도 된다(앞에 군더더기가 붙은 파일이 있다) — 앞 1KB에서 찾는다
  const isPdf = buf.subarray(0, 1024).toString("latin1").includes("%PDF-");
  if (isPdf) {
    if (ext !== "pdf") throw new UnsupportedDocumentError("파일 내용은 PDF인데 확장자가 다릅니다. .pdf 파일만 지원합니다.");
    return "pdf";
  }
  if (isOle) {
...
/** 동기 파서(hwp·hwpx·docx). xlsx(exceljs)·pdf(pdf.js) 읽기는 비동기라 parseDocumentAsync를 쓴다. */
export function parseDocument(buf: Buffer, fileName: string): DocumentModel {
  ...
  throw new UnsupportedDocumentError("xlsx·pdf는 parseDocumentAsync로 읽습니다.");
}
...
export async function parseDocumentAsync(buf: Buffer, fileName: string): Promise<DocumentModel> {
  ...
  return parseDocument(buf, fileName);
}
