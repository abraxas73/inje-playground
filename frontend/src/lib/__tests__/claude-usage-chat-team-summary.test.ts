...
describe("aggregateChatTeams", () => {
  it("팀별로 수치를 더하고 인원은 고유 이메일로 센다(여러 Claude 조직 계정 중복 제거)", () => {
    const rows = aggregateChatTeams([
      { ...base, email: "a@x.com", days_active: 5, messages: 100, chats: 3, team: "T", parent_unit: "C", office_turns: 4 },
    ...
    expect(t).toMatchObject({ parent: "C", users: 2, active_users: 1, messages: 120, chats: 4, office_turns: 4 });
    expect(rows.find((r) => r.team === "명부 없음")!.office_turns).toBe(0);
