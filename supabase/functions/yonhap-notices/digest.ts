...
/** 인사·부고 소식 메일(예약·지금 수신·미리보기) 템플릿. 프런트 미리보기도 이 함수 결과를 그대로 보여준다. */
...
export function buildDigest(notices: DigestNotice[], total: number, asOf: string, lastSyncedAt: string | null, appUrl: string, periodFrom?: string): Digest {
  ...
  const html = `<div lang="ko" style="max-width:640px;margin:auto;padding:24px;font-family:Arial,sans-serif;color:#0f172a"><h1 style="font-size:24px">연합뉴스 인사·부고</h1><p>${escape(intro)}</p><p style="font-size:12px;color:#64748b">${escape(period)}</p><p style="font-size:12px;color:#64748b">${escape(collection)}</p><ul style="list-style:none;padding:0">${rows.map((r) => r.html).join("")}</ul><p><a href="${escape(pageUrl)}">전체 소식 보기 · 수신 시간 변경 · 수신 해제</a></p><p style="font-size:12px;color:#64748b">연합뉴스 RSS 제공 소식입니다. 자세한 내용과 정정 사항은 원문을 확인해 주세요. 예약 메일은 하루 한 번, 즉시 수신은 요청할 때 발송합니다.</p></div>`;
  const text = `${subject}\n\n${intro}\n${period}\n${collection}\n\n${rows.map((r) => r.text).join("\n\n")}\n\n전체 소식 보기 · 수신 시간 변경 · 수신 해제: ${pageUrl}\n출처: 연합뉴스 RSS. 자세한 내용은 원문을 확인해 주세요.`;
  return { subject, html, text };
}

/** [from, to) 사이에 **저장된**(created_at) 소식을 최신순 100건까지 담는다. 송고 시각 기준이 아니라 늦게 수집된 기사도 빠지지 않는다. */
export async function loadDigest(admin: SupabaseClient, from: string, to: string, appUrl: string): Promise<{ digest: Digest; count: number }> {
  ...
  return { digest: buildDigest((articles.data ?? []) as DigestNotice[], count, to, sync.data?.finished_at ?? null, appUrl, from), count };
}
